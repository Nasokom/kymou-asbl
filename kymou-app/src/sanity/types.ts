...
export type AllSanitySchemaTypes = SanityImagePaletteSwatch | SanityImagePalette | SanityImageDimensions | SanityFileAsset | Geopoint | BlockContent | Content | Contact | CustomMedia | HomePage2 | BlogContent | BlogPost | Projectv2 | TextImg | SanityImageCrop | SanityImageHotspot | SanityImageAsset | SanityAssetSourceData | SanityImageMetadata | Settings | MediaTag | Slug;
...
// Variable: POST_QUERY
// Query: *[_type == "blogPost" && slug.current == $slug][0]{  _id,  'slug':slug.current,  title,  author,  content,   publishedAt,  _createdAt,  _updatedAt,  description,  hero{    ...,    asset->{      ...    }  },}
...
// Query: *[_type == "projectv2" && slug.current == $slug][0]                    {title,                    _id,                    _createdAt,                    _updatedAt,                     hero{    ...,    asset->{      ...    }  },                    'description':pt::text(pitch),                    slug,                    pitch,                    problem,                    action,                    result,                  'gallery':gallery[].asset->{  originalFilename,  url,  title,  description,       'lqip':metadata.lqip,    'dimensions':metadata.dimensions,  altText,  _rev,}}
